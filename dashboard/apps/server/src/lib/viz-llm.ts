...
export async function llmRecommendChartsForTables(tables: string[], goal?: string, llmType?: string): Promise<ChartDefinition[]> {
  ...
  if (type === 'openai') {
    ...
    const resp = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'api-key': cfg.apiKey }, body: JSON.stringify({ messages: [ { role: 'system', content: guidance }, { role: 'user', content: user } ], temperature: 0 }) })
    if (!resp.ok) throw new Error(await resp.text())
    const data = await resp.json()
    content = data?.choices?.[0]?.message?.content?.trim() || ''
  } else if (type === 'openai_compatible') {
    const client = new OpenAI({ apiKey: cfg.apiKey, baseURL: cfg.baseUrl })
    const completion = await client.chat.completions.create({ model: cfg.model, temperature: 0, messages: [ { role: 'system', content: guidance }, { role: 'user', content: user } ] })
    content = completion.choices?.[0]?.message?.content?.trim() || ''
  } else if (type === 'anthropic') {
    const url = 'https://api.anthropic.com/v1/messages'
    const resp = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': cfg.apiKey, 'anthropic-version': cfg.version || '2023-06-01' }, body: JSON.stringify({ model: cfg.model, max_tokens: 1024, system: guidance, messages: [ { role: 'user', content: user } ] }) })
    if (!resp.ok) throw new Error(await resp.text())
    const data = await resp.json()
